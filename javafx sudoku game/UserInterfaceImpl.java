...
public class UserInterfaceImpl implements IUserInterfaceContract.View,EventHandler<KeyEvent> {
    ...
    private static final Color WINDOW_BACKGROUND_COLOR = Color.rgb(0, 77, 150);
    ...
    private void addRestartButton(Group root2) {
       javafx.scene.control.Button restartButton = new javafx.scene.control.Button("Restart");
    restartButton.setLayoutX(280);   // adjust if needed
    restartButton.setLayoutY(650);   // adjust if needed
...
public void updateBoard(SudokuGame game) {
    for (int xIndex = 0; xIndex < 9; xIndex++) {
        for (int yIndex = 0; yIndex < 9; yIndex++) {
            ...
            // ----------------------
            // COLOR DIFFERENCE ADDED
            // ----------------------
            if (game.getGameState() == GameState.NEW) {

                if (value.equals("")) {
                    // USER INPUT CELL (empty initially)
                    tile.setStyle("-fx-opacity: 1; -fx-text-fill: black;");
                    tile.setDisable(false);

                } else {
                    // ORIGINAL GIVEN NUMBER
                    tile.setStyle("-fx-opacity: 0.8; -fx-text-fill: blue;");
